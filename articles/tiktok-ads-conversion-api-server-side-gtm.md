...
:::message
本番環境への公開前に、テストイベントの受信を確認してから公開するようにしましょう。イベント名やパラメータに誤りがあると、TikTokの広告最適化に活用されないデータが送信され続けることになります。
:::
...
関連記事: [TikTok広告のコンバージョンAPIをサーバーサイドGTMで実装する手順](https://logical-web.jp/blog/tiktok-ads-capi-server-side-gtm/?utm_source=zenn&utm_medium=article&utm_campaign=related_article)
