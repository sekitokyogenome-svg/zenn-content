...
**リージョンと接続は共通**
ここは変わりません。既に `ML.GENERATE_TEXT` が動いているなら、接続とその権限は流用できます。第2回の設定作業をやり直す必要はありません。

**コスト構造も変わらない**
どちらも行ごとにモデルを呼びます。AI 関数は**処理した行数で課金される**ため、`LIMIT` から始める・前段で絞る・結果をテーブルに保存する・差分処理にする、という原則はそのまま適用されます。
...
| 第2回 | IAM と接続。権限が必要な主体は2つある |
| 第3回 | Knowledge Catalog。文脈を供給しないと精度は上がらない |
| 第4回 | 世代の違いと移行 |
