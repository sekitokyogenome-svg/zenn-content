...
BigQueryのコンソールでクエリを実行する前に、右上に表示される「このクエリは約〇〇MBを処理します」という表示を事前に確認する習慣をつけてください。実行前にコストを把握できるため、想定外の課金を防げます。
...
分析に使わなくなった2年以上前のデータは、GCS（Google Cloud Storage）にエクスポートしてBigQueryから削除することも一つの選択肢です。そのまま残しても長期保存ストレージの割引が自動適用されるため、削除せずにコストを抑える方法も有効です。長期保存ストレージは通常の半額程度の料金になるため、削除しなくても一定のコスト最適化が見込めます。
...
1. **クエリには日付範囲を指定する（基本原則）** — `_TABLE_SUFFIX`を使って対象テーブルを絞り込み、不必要なスキャンを防ぐ
