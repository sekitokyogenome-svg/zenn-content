...
生成されたSQLはあくまで「下書き」です。実行前に必ずカラム名・テーブル名・日付範囲を確認し、ご自身の環境に合わせて修正してください。
...
`_TABLE_SUFFIX`を使ったワイルドカードクエリの場合、日付の指定ミスによって意図しない範囲のデータを読み込む可能性があります。特に大きなテーブルではコスト増につながるため、実行前に必ず確認してください。
