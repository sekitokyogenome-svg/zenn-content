...
このプロンプトのポイントは3点あります。**判定基準（±20%）を数値で明示**すること、**仮説を3カテゴリで網羅性を確保**すること、**確認方法まで出力**させて次のアクションに直結させることです。
...
Claude Codeの出力をSlackやメールに転送する仕組みを組み合わせることで、毎週月曜の朝に先週の異常値サマリーを受け取る運用も実現できます。自動化パイプラインの例として、BigQueryスケジュールクエリでCSVを出力し、PythonスクリプトでClaude APIを呼び出してSlackに通知する構成が挙げられます。
