...
emoji: "🔍"
type: "tech"
topics: ["claudecode", "bigquery", "dataanalysis"]
published: false
---
...
```text
BigQueryのbeeracle.beeracle_mart.mart_channel_performanceから、
今月と先月の売上合計を比較してください。
差額と変化率も計算してください。
```
...
WITH current_period AS (
  ...
  FROM `beeracle.beeracle_mart.mart_channel_performance`
...
```markdown
<!-- .claude/commands/revenue-diagnosis.md -->
BigQueryのbeeracle_martから以下の分析を実行してください。
...
売上が下がったとき、原因を手作業で調べるのは時間がかかるうえに属人的です。Claude Code × BigQuery MCPを使えば、期間比較からディメンション分解、仮説生成までを数分で完了できます。
...
👉 [GA4×BigQuery基盤構築サービス](https://coconala.com/services/554778)
